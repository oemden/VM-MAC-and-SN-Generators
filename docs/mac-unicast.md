...
- **Why this project only generates unicast MACs**: Using multicast-style MAC addresses for VM NICs can confuse network equipment and is almost never desired. The `genmac` script therefore always generates **unicast** MAC addresses and avoids multicast ranges by design.
